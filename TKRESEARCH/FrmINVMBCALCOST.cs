        ...
        public StringBuilder SETSQL3(string YM, string MB001, string MB002,string MB003)
        {
            ...
            SB.AppendFormat(@"   
                           ...
                           SELECT MC001,MC004,MD003,MD006,MD007
                            ,ISNULL((SELECT AVG(NULLIF(ME007+ME008+ME009+ME010,0)/NULLIF(ME003+ME005+ME004,0)) FROM [TK].dbo.CSTME WHERE  ME001=MD001 AND ME002 LIKE '2022%'),0) AS '總成品平均成本'
                            ,ISNULL((SELECT AVG(NULLIF(ME007,0)/NULLIF(ME003+ME005+ME004,0)) FROM [TK].dbo.CSTME WHERE  ME001=MD001 AND ME002 LIKE '2022%'),0) AS '材料平均成本'
                            ,ISNULL((SELECT AVG(NULLIF(ME008,0)/NULLIF(ME003+ME005+ME004,0)) FROM [TK].dbo.CSTME WHERE  ME001=MD001 AND ME002 LIKE '2022%'),0) AS '人工平均成本'
                            ,ISNULL((SELECT AVG(NULLIF(ME009,0)/NULLIF(ME003+ME005+ME004,0)) FROM [TK].dbo.CSTME WHERE  ME001=MD001 AND ME002 LIKE '2022%'),0) AS '製造平均成本'
                            ,ISNULL((SELECT AVG(NULLIF(ME010,0)/NULLIF(ME003+ME005+ME004,0)) FROM [TK].dbo.CSTME WHERE  ME001=MD001 AND ME002 LIKE '2022%'),0) AS '加工平均成本'
                            ...
                            ,ISNULL((SELECT SUM (NULLIF(MD006,0)/NULLIF(MD007,0)) FROM [TK].dbo.BOMMC MC, [TK].dbo.BOMMD MD ,[TK].dbo.INVMB MB WHERE  MC.MC001=MD.MD001 AND MD.MD003=MB.MB001 AND MC.MC001=BOMMC.MC001 AND (MB.MB001 LIKE '3%' OR MB.MB001 LIKE '4%')),0)  AS '總半成品重'
                            FROM [TK].dbo.BOMMC
